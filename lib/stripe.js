...
/**
 * Create a PaymentIntent specifically for auction winners
 * Includes metadata for tracking and automatic payment method support
 * @param {number} amount - Amount in dollars
 * @param {string} winnerId - User ID of the auction winner
 * @param {string} listingId - Listing ID
 * @param {string} curatorId - Curator ID
 * @returns {Promise<Stripe.PaymentIntent>}
 */
export async function createAuctionPaymentIntent(amount, winnerId, listingId, curatorId) {
  const stripe = getStripe();
  const amountInCents = Math.round(amount * 100);
...
/**
 * Charge a user's saved payment method (for auction winners)
 * @param {number} amount - Amount in dollars
 ...
 * @returns {Promise<Stripe.PaymentIntent>}
 */
export async function chargeCustomer(amount, customerId, paymentMethodId, metadata = {}) {
  const stripe = getStripe();
  const amountInCents = Math.round(amount * 100);
  ...
  const paymentIntent = await stripe.paymentIntents.create({
    ...
    off_session: true,
    confirm: true, // Immediately charge the card
    metadata,
  });
...
/**
 * Create a pre-authorization PaymentIntent (hold funds without capturing)
 * Used when a user places a claim to ensure they have funds available
 * @param {number} amount - Amount in dollars to hold
 * @param {string} customerId - Stripe customer ID
 * @param {string} paymentMethodId - Saved payment method ID
 * @param {object} metadata - Payment metadata
 * @returns {Promise<Stripe.PaymentIntent>}
 */
export async function createPreAuthPaymentIntent(amount, customerId, paymentMethodId, metadata = {}) {
  const stripe = getStripe();
  const amountInCents = Math.round(amount * 100);
  ...
  const paymentIntent = await stripe.paymentIntents.create({
    ...
    off_session: true,
    confirm: true, // Confirm to create the hold
    metadata: {
      ...metadata,
      type: 'pre_auth',
    },
 ...
 * Called when user is outbid
 * @param {string} paymentIntentId - The PaymentIntent ID to cancel
 * @returns {Promise<Stripe.PaymentIntent>}
 */
export async function cancelPreAuth(paymentIntentId) {
  const stripe = getStripe();
  return await stripe.paymentIntents.cancel(paymentIntentId);
}

/**
 * Capture a pre-authorized payment (actually charge the card)
 * Called when auction ends and user wins
 * @param {string} paymentIntentId - The PaymentIntent ID to capture
 * @param {number} amountToCapture - Optional: capture less than authorized amount (in dollars)
 * @returns {Promise<Stripe.PaymentIntent>}
 */
export async function capturePreAuth(paymentIntentId, amountToCapture = null) {
  const stripe = getStripe();
  const params = {};
...
/**
 * Update a PaymentIntent's amount (for applying coin discounts)
 * @param {string} paymentIntentId - The PaymentIntent ID
 * @param {number} newAmount - New amount in dollars
 * @param {object} metadata - Additional metadata to add
 * @returns {Promise<Stripe.PaymentIntent>}
 */
export async function updatePaymentIntentAmount(paymentIntentId, newAmount, metadata = {}) {
  const stripe = getStripe();
 ...
 * @param {string} paymentIntentId - The PaymentIntent ID to refund
 * @param {number|null} amount - Amount to refund in dollars (null for full refund)
 * @param {string} reason - Reason for refund ('requested_by_customer', 'duplicate', 'fraudulent')
 * @param {object} metadata - Additional metadata
 * @returns {Promise<Stripe.Refund>}
 */
export async function refundPayment(paymentIntentId, amount = null, reason = 'requested_by_customer', metadata = {}) {
  const stripe = getStripe();
