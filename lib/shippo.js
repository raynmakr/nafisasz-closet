...
export async function createShipment(fromAddress, toAddress, parcel) {
  ...
  if (fromAddress.street1?.toLowerCase() === toAddress.street1?.toLowerCase() &&
      fromAddress.zip?.replace(/\s/g, '').toLowerCase() === toAddress.zip?.replace(/\s/g, '').toLowerCase()) {
    throw new Error('Cannot ship to the same address. Buyer and seller addresses must be different.');
  }
...
export async function purchaseLabel(rateId) {
  ...
  if (result.status === 'ERROR') {
    ...
    } else if (errorMessages.some(m => m?.toLowerCase().includes('email') || m?.toLowerCase().includes('phone'))) {
      friendlyMessage = 'Phone and email are required for shipping labels. Please update your shipping addresses.';
    } else if (errorMessages.some(m => m?.toLowerCase().includes('address'))) {
      friendlyMessage = 'Address validation failed. Please verify the shipping address is correct.';
    }
